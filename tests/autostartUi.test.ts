...
test('wires launch-at-login through the top-right app shell control', () => {
  ...
  assert.strictEqual(appSource.includes("invoke<boolean>('is_windows_autostart_enabled')"), true);
  assert.strictEqual(appSource.includes("invoke<boolean>('set_windows_autostart_enabled', { enabled })"), true);
  assert.strictEqual(appSource.includes('return isAutostartEnabled();'), true);
  ...
  assert.strictEqual(appSource.includes('<StartupToggle language={language} platform={platform} />'), true);
...
test('registers the Tauri autostart plugin and permissions', () => {
  ...
  const commandSource = readFileSync('src-tauri/src/commands/autostart.rs', 'utf8');
  ...
  assert.strictEqual(cargoSource.includes('winreg = "0.10"'), true);
  ...
  assert.strictEqual(libSource.includes('commands::autostart::is_windows_autostart_enabled'), true);
  assert.strictEqual(libSource.includes('commands::autostart::set_windows_autostart_enabled'), true);
  assert.strictEqual(commandSource.includes('SOFTWARE\\\\Microsoft\\\\Windows\\\\CurrentVersion\\\\Run'), true);
  assert.strictEqual(commandSource.includes('WINDOWS_AUTOSTART_ARG'), true);
  assert.strictEqual(commandSource.includes('quote_windows_command_arg(&exe_path.display().to_string())'), true);
  assert.strictEqual(libSource.includes('launched_from_windows_autostart()'), true);
